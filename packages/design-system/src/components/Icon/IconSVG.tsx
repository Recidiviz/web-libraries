...
// `StrokedIcon` remaps its `fill` prop to the SVG `stroke` attr
const StrokedIcon = ({ children, viewBox }: IconSVGProps) => {
  const { fill, ...props } = React.useContext(IconSVGContext);
  return (
    <BaseSVG
      {...props}
      viewBox={viewBox}
...
const FilledIcon = ({ children, viewBox, ...rest }: IconSVGProps) => {
  const props = React.useContext(IconSVGContext);
  return (
    <BaseSVG viewBox={viewBox} {...props}>
      {children}
    </BaseSVG>
  );
};
